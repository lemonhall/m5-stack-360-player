...
- **Scope**: 使用本地 JSON 配置 `config/local.vlc-player.json`；缺失时自动使用默认值；保存时自动创建目录；默认启用 `inject_spherical_metadata`，缓存目录默认为 `config/vr-cache`。
